...
def evaluate(model, datadl, device="cpu"):
    ...
    with torch.no_grad():
        for x, y in datadl:
            x = x.to(device)
            y = y.to(device).long()
            predictions.extend(model(x[:,:50].long(),x[:,50:]).tolist())
    ...
    def train_step(self, train_iter,epochs=None, plot_picture=True):
        ...
        for epoch in range(1, epochs+1):
            ...
            for train_data, train_label in train_iter:
                ...
                train_data, train_label = train_data.to(self.device), train_label.to(self.device)
                y_hat = self.model(train_data[:,:50].long(),train_data[:,50:])
            ...
            y_plot.append(self.lr_scheduler.base_lr_orig)
            x_plot.append(epoch)
            # y_plot.append(self.lr_scheduler(epoch))
            # epochTrainLoss.append(train_loss)
            finish = time.time()
        ...
        if plot_picture:
            ...
            plt.xlabel('step')
            plt.ylabel('lr')
            plt.savefig('./result/bert+Cos_warmup.jpg')
            plt.show()

            # 绘制损失函数曲线
            plt.figure()
            plt.plot(range(1, epochs+1), epochTrainLoss)
            plt.legend(['trainLoss'])
            plt.xlabel('epochs')
            plt.ylabel('SHLoss')
            plt.savefig('./image/bert+Cos_warmup_Loss(200).jpg')
            plt.show()
